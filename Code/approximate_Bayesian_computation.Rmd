...
invisible(pblapply(1:nrow(pop_map), function(x){
  ...
  if(x != nrow(pop_map)){
    cat(paste(pop_map[x, 1], pop_map[x, 2], collapse = "\t"), "\n", file = tmp_nm, sep = "", append = TRUE)
  } else {
    cat(paste(pop_map[x, 1], pop_map[x, 2], collapse = "\t"), file = tmp_nm, sep = "", append = TRUE)
  }
...
# 180k simulations...why? I thought I did 10k simulations per model, so there should be 60k. Why are there 3x more?
# The OOB error rate is 0%, suspiciously perfect 
barc_RF
...
observedSFS <- file.path(tmp_path, "BARC_model_MSFS")
traits <- file.path(tmp_path, traitsfile)

# From earlier in delimitR tutorial: The user must also provide a vector of sample sizes. Again, this must be compatible with the traits file and the observed SFS. The sample sizes should be specified in order from population 0 to population n-1. These should be the numbers AFTER downsampling.
...
# Looks like I should probably rerun BARC fsc26 simulations with these sample sizes, and then redo priors, model training
