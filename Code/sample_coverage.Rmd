...
PE_optim <- c(
  "PE_lib2_NAT_ELGE_NMSU-208_CCGTGAGA", "PE_lib1_NAT_BCAR_NMSU-279_AGTCAAGC", "PE_lib1_INV_MALL_37_GCTAACGA",
...
Make a population map per SE and PE library that will be used for Stacks optimization. The population maps should have two columns: 1) the sample name (prefix, no extension) and 2) integer or string indicating population. Here, I will use regions as populations, since this is how I chose samples for optimization.
...
invisible(pblapply(1:length(PE_optim), function(i){
  ...
  reg <- meta_dats %>%
    filter(Sample_Name == PE_optim[i]) %>%
    pull(Region)
  
  # If not on the last individual, write out a new line symbol to start the next sample on a new line
  if(i != length(PE_optim)){
    tmp_line <- paste(paste(paste(PE_optim[i], ".fil", sep = ""), reg, sep = "\t"), "\n", sep = "")
  } else {
    tmp_line <- paste(paste(PE_optim[i], ".fil", sep = ""), reg, sep = "\t")
  }
